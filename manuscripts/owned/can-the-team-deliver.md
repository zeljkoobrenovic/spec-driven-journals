...
The next question is why a small country change takes three weeks when commit-to-production takes under two days, and how much of that is approvals, the two specialists’ queue or implementation. [Capacity Is Not Headcount: Trace the Work Before You Hire](#fix-decisions-before-hiring) answers it by tracing one of these pricing changes from request to production, and decides whether Larkspur should change roles, hire, or both. Only then should Larkspur choose among configuring a supplier, drawing a boundary it owns around the country rules and replacing the core; [Turn “We Expect Growth” Into a Design Decision](#growth-into-design) makes that choice against this same finding.
