...
writes the generated PNG next to the post.

Existing files are skipped by default. Use ``--overwrite`` to regenerate.
Standard library only, matching the journal wiring scripts.
"""
...
def call_gemini(api_key: str, model: str, prompt: str, args: argparse.Namespace) -> bytes:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for target in targets:
        ...
        image_bytes = call_gemini(api_key, args.model, target.prompt, args)
