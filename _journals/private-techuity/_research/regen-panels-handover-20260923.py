#!/usr/bin/env python3
"""Regenerate or re-letter legacy comic panels of 33-handover-of-obligations (asset slug 25-handover-of-obligations).
...
cost), HND-002 (panel 4's arrows sent company shares to the fund and mixed
secondary and fund-interest sales; it also showed dollar signs), HND-007
(panel 5's CONTRACTUAL tray becomes BINDING). These panels carry exact
labels, so, like the financing chapter's script, this one prints the labels
quoted in the scene and forbids any other text. The scene, dialogue and
caption come from the panel JSON in comics.md; replaced images are archived
under `_research/discarded-comic-variants/`.

Usage:
  python3 regen-panels-handover-20260923.py 02-scene [03-scene ...]
  python3 regen-panels-handover-20260923.py --retext 05-scene "OLD WORDS" "NEW WORDS"
"""
...
def retext(panel_id: str, old: str, new: str, key: str) -> None:
    ...
    (backups / f'33-handover-of-obligations-comic-{panel_id}-{hashlib.sha256(previous).hexdigest()[:12]}.jpeg').write_bytes(previous)
