#!/usr/bin/env python3
"""Round-2 driver: regenerate the Part VI chapter overview (Figure 1 of part-8-intro)
with a qualified Toys R Us question.

In-depth review 23 September 2026, round 2, finding P6-003: the round-1 stacked render
asked "Why did reported profit leave so little cash?", which neither names the earnings
measure (Toys R Us reported positive operating earnings but an overall net loss) nor
distinguishes cash generated by operations (about nil in fiscal 2016) from cash held
(the company still held $566 million at year end). This driver keeps the round-1 layout,
labels and recipe (stacked column, explanation lines at title size, Pillow crop with a
20-pixel margin, python3.11) and changes only card 3's explanation to
"Why did profit before borrowing costs and tax generate almost no cash?", plus the
matching alt text. Same usage as the round-1 driver.
"""
import argparse
import os
import sys
...
PROMPT = (
    ...
    "short lines of at most four words so it never shrinks (card 3's explanation therefore "
    "runs onto three lines). Fill the column; do not leave "
    ...
    "Card 3 title: \"Toys R Us\"; explanation: \"Why did profit before borrowing costs and tax generate almost no cash?\" "
...
ALT = (
    ...
    "and tax generated almost no cash; TeamSystem, what commitments pass to each new owner. A bracket gathers the four "
