#!/usr/bin/env python3
"""Regenerate legacy comic panels of 31-teamsystem (asset slug 29-teamsystem).
...
lettering. The panel is re-rolled with the two record labels quoted in the
scene so they print large and legible; every other word is forbidden. The
journal wrapper `generate_comics.py` asserts that asset paths match the
post's folder slug; this post was renumbered after the illustrated edition,
so its assets still live under `29-teamsystem/`. The scene, dialogue and
caption come from the panel JSON in comics.md; the replaced image is
archived under `_research/discarded-comic-variants/`.

Usage: python3 regen-panel-teamsystem-20260923.py 06-scene
"""
...
def main() -> int:
    ...
    for panel_id in ids:
        ...
        (backups / f'31-teamsystem-comic-{panel_id}-{hashlib.sha256(previous).hexdigest()[:12]}.jpeg').write_bytes(previous)
