#!/usr/bin/env python3
"""Regenerate legacy comic panels of 26-anatomy-of-a-layoff (asset slug 24-anatomy-of-a-layoff).
...
allocation is 4/2/5/3/2. The panel scene now puts the count in each label and
asks for exactly that many figures. Adapted from
`regen-panels-financing-slipped-20260923.py` (same prompt builder, identities
and helper); only the post paths and the archive name change. The scene,
dialogue and caption come from the panel JSON in comics.md.

Usage: python3 regen-panels-anatomy-of-a-layoff-20260923.py 02-scene
"""
...
POST = J / 'posts/26-anatomy-of-a-layoff'
...
def main() -> int:
    ...
    for panel_id in ids:
        ...
        (backups / f'26-anatomy-of-a-layoff-comic-{panel_id}-{hashlib.sha256(previous).hexdigest()[:12]}.jpeg').write_bytes(previous)
