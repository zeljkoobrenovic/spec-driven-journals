#!/usr/bin/env python3
"""One-off driver: regenerate the Part V chapter overview as a stacked column.

In-depth review 23 September 2026, round 1, findings P5-002 and P5-004: the
22 September two-lane 4:3 render (sha f7b5b48d..., archived under
_research/discarded-illustration-variants/part-5-intro-chapter-overview-f7b5b48dc12e.jpeg)
used shorthand a newcomer cannot follow ("Use diligence" / "Record what changes
the plan", "Carry evidence and obligations") and, at the 342-pixel content width
of a 390-pixel phone, its subtitles and the conditional "If a reduction is
required" label fell to roughly 6-9 pixels. This version keeps the two separate
scenarios (three cards following the onboarding finding, two cards for the
separate funding-delay scenario joined only by a dashed conditional arrow), the
LEAD heading and the ochre purpose banner, but stacks all five cards in one
column on a tall 9:16 canvas with the explanation lines at title size, and
crops the empty paper margins with Pillow, as the Part I overview was
regenerated the same day (regen-figure-part-1-overview-20260923-r2.py).
Driven directly through the illustrator module. Run with python3.11.

Outcome: render 1 (768x1376, sha noted in the revision log) was legible at phone
width but drew the "same finding" teal rail down all five cards, past the
separator, so the prompt now forbids any edge rail; see the revision log for
the accepted render.
"""
import argparse
import os
...
PROMPT = (
    ...
    "Group label 1, above the first card: \"Follow the onboarding finding\".\n"
    ...
    "\"revise cash and commitments\". Small line-drawn hourglass beside coins at the left.\n"
    ...
    "Exactly five cards, two group labels, one dashed arrow with its label and one banner; every "
...
ALT = (
    "Five chapters stacked in one column under the heading LEAD. Group one, Follow the onboarding "
    "finding: Use diligence (check the business before the deal is signed) leads to Plan the first "
    ...
    "funding delays (expected money is late: revise cash and commitments) is joined to Plan layoffs "
