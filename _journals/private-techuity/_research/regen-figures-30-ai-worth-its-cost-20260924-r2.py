#!/usr/bin/env python3
"""Round-2 figure driver for 30-ai-worth-its-cost (in-depth review, 24 September 2026).

AIC-004: Figure 2 card 3 lettering "Cash contribution" -> "Contribution" and
"after running cost, before payroll" -> "after supplier cost, before payroll"
(Gemini lettering edit on the existing image, as regen-figures-scale-the-team-with-ai-20260924.py).
AIC-015: Figures 1 and 3 are rendered from numeric coordinates by
render-figures-30-ai-worth-its-cost-20260924.py; this driver installs them.

Usage: --generate fig2 writes /tmp/aic-figures/three-numbers-never-added.jpeg;
--accept <ids> installs candidates from /tmp/aic-figures, archives the old
images under _research/discarded-illustration-variants/ and updates the prompt archive.
"""
...
RENDERER = 'render-figures-30-ai-worth-its-cost-20260924.py'

EDIT_PROMPT = ("Edit the attached illustration. Change ONLY two pieces of lettering, both on the right-hand card: "
               "replace the heading words \"Cash contribution\" with the single word \"Contribution\", and replace the "
               "bottom line \"after running cost, before payroll\" with \"after supplier cost, before payroll\", in the same "
               "places, same lettering style, size and color, on as many lines as the new words need so that they stay "
               "clear of every other element. Keep every other pixel of the picture as it is: the same three cards, the "
               "crossed-out plus signs, the clock, documents, price tag, card, coins, cash box, colors and all other text. "
               "Add nothing else.")
...
TARGETS = {
    ...
    'fig2': dict(key='three-numbers-never-added', mode='edit', prompt=EDIT_PROMPT,
                 alt="Three cards separated by crossed-out plus signs. Customer value: €540 a month, planner time freed, before the fee. Revenue: €57,000, April to September 2027. Contribution: about €26,900, after supplier cost, before payroll.",
                 expect=['Customer value', '€540 a month', 'planner time freed, before the fee', 'Revenue', '€57,000', 'April to September 2027', 'Contribution', 'about €26,900', 'after supplier cost, before payroll']),
...
def accept(tid: str, note: str) -> None:
    ...
    if t['mode'] == 'render':
        ...
        e['prompt'] = e['prompt'].replace("'Cash contribution'", "'Contribution'").replace("'after running cost, before payroll'", "'after supplier cost, before payroll'").replace("Cash contribution, about €26,900, after running cost, before payroll", "Contribution, about €26,900, after supplier cost, before payroll")
    ...
    if t['key'] == 'three-numbers-never-added':
        ...
        e['caption'] = "Three answers to what the sorting feature is worth, and none of them adds to the others. Customer value is the estimated worth of the planner time a customer frees, before the fee, and is not money Larkspur receives; revenue is Larkspur's money before costs; contribution is what remains after the supplier running cost for the stated period, each month's bills counted in that month, before the salaries already in payroll."
