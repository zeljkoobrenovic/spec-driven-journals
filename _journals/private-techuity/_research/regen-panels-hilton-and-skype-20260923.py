#!/usr/bin/env python3
"""Regenerate or re-letter legacy comic panels of 28-hilton-and-skype (asset slug 26-hilton-and-skype).
...
panel 4 carried "SKYPE IP DISCLOSURES", "OPERATING RIGHTS" and an invented
"GLOBAL PEER DATABASE" architecture diagram) and HSK-007 (panel 5 labelled the
two prices "COMPANY A PRICE" / "COMPANY B PRICE" although both are Skype's
value at purchase and at its later sale). Like the handover chapter's script,
this one prints only the labels quoted in the scene and forbids any other
text. The scene, dialogue and caption come from the panel JSON in comics.md;
replaced images are archived under `_research/discarded-comic-variants/`.
The journal's generate_comics.py cannot run on this post because its asset
folder keeps the legacy `26-` slug.

Usage:
  python3 regen-panels-hilton-and-skype-20260923.py 01-scene [04-scene ...]
  python3 regen-panels-hilton-and-skype-20260923.py --retext 05-scene "OLD WORDS" "NEW WORDS"
"""
...
POST = J / 'posts/28-hilton-and-skype'
...
def archive(asset: Path, panel_id: str) -> None:
    ...
    (backups / f'28-hilton-and-skype-comic-{panel_id}-{hashlib.sha256(previous).hexdigest()[:12]}.jpeg').write_bytes(previous)
