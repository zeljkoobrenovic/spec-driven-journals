...
Implemented from the run `.in-depth-reviews/20260923-160608.PjF9JD` (round-01 review). The summary overview image was regenerated; the six comic panels and the two article figures were inspected and left unchanged.

| Finding | Disposition |
| --- | --- |
| VISMA-006 (medium) dense passages: 2023 announcement, report definitions, comic introduction | **Fixed.** The 2023 paragraph is now three: the secondary sale and the €19 billion valuation; who took part and with how much (new investors and their €1 billion, existing shareholders and their €3 billion, Hg’s majority stake); the SaaS focus, with the citation. Organic growth (with its “not simply subtracting acquired revenue” caveat), free cash flow (with the before-tax caveat) and ARR (with the per-payslip caveat) each have a paragraph; the Q4 report is cited on the first and last. The comic introduction is split after the ownership explanation; the EBITDA/adjusted definition now sits immediately before panel 5, the earnings panel, as a short standalone paragraph. |
| VISMA-011 (low) residual jargon in standalone formats and artwork | **Fixed.** Holding period translated at first use in the summary (“how long its investors expect to keep it before selling”), the comic introduction and panel 2’s caption; the comic introduction describes the shared systems (sign-in, billing, hosting); panel 6 says “more than half the shares” and glosses integration work. Article: NPS expanded as South Korea’s National Pension Service; capital spending and operating earnings glossed in the Toys R Us handoff; securities and bonds glossed in the ESMA annotation. Summary: KKR and Cinven described as other investment firms; buyout as buying enough shares to control the company. Overview image regenerated with `_research/regen-summary-visual-visma-20260923.py`: RECONCILIATION, OPERATING DESIGN and the other small thread labels are gone; the image carries exactly LOCAL DECISIONS, SHARED SUPPORT, CHANGING OWNERSHIP and ONE YEAR, TWO PROFIT FIGURES (opened and inspected; no numbers or stray text). Caption and alt text rewritten to spell out the three things to examine; archive entry, previous prompt and hash recorded in `summary-visual-prompts-20260913.json`, `artwork-review-20260913.json` updated, previous image archived under `_research/discarded-summary-variants/`. Summary held at 500 prose words by the collection’s counting rule after trimming elsewhere. |
| VISMA-013 (medium) EBITDA bridge amounts off-screen on a phone | **Fixed.** The bridge table is wrapped in the existing `<div class="table-compact">` treatment and its row labels shortened (“EBITDA, 2024 annual report”; “Add back M&A expenses (2025 annual report’s reconciliation)”; “Adjusted EBITDA for 2024, 2025 annual report”). Rendered with Playwright at 1280, 390 and 360 px: at both phone widths the table is 342 / 312 px wide, not scrollable, and every cell including the € million heading and all three amounts is inside the viewport; the ownership map still scrolls (560 px in a 342 px wrap). |

Checks run: 892.646 + 11.665 = 904.311 unchanged; comic JSON parses (6 panels, captions match rendered lines, image hashes match files); `python3 _wiring/build.py` (`[built] private-techuity`); no unresolved `[[…]]` and every local reference in `docs/private-techuity/visma.html` resolves; Playwright screenshots of the bridge (1280/390/360 px), the TL;DR figure and caption (390 px) and comic panels 4–5 with the new EBITDA paragraph (390 px). Manuscript: exported to a scratch directory (35 other chapters differ because their sources are mid-edit in other sessions), only `visma.md` installed with the heading kept as “# 31.” and its two manifest entries updated; the validator reports the same 11 pre-existing errors for other chapters, none for Visma. The build also refreshed `teamsystem`, `toys-r-us` and `success-for-whom` outputs that other sessions had already left modified; they now reflect those sessions’ current sources. Still unverified: the three original Visma PDFs (cached text extractions only).

## In-depth review round 4 (23 September 2026): dispositions

Implemented from the run `.in-depth-reviews/20260923-160608.PjF9JD` (round-02 review). Only `summary.md` changed; the article, comic, spec criteria and all eleven images were left as reviewed.

| Finding | Disposition |
| --- | --- |
| VISMA-006 (low) compressed 2017 sentence in the summary; KKR/Cinven descriptor after the fact; buyout gloss attached to Hg’s 41% | **Fixed.** The 54-word sentence is now four: the two Hg funds; the buyout, glossed as “a purchase giving a group of buyers control”; “Investment firms KKR and Cinven were to sell, KKR all its shares and Cinven 40% of its own holding, subject to regulatory approval”; “Hg led the buying group and would hold 41% of the shares” (announced terms, not confirmed completion). December 2023 starts its own paragraph. To stay at 500 prose words, the redundant “(rounded: €893 million and €904 million)” and “on paper” were dropped from the EBITDA paragraph; the figures, add-back warning and every qualification remain. |

Checks run: prose word count 500 (image text, caption and citations excluded); `python3 _wiring/build.py` changed only `docs/private-techuity/visma.html` (docs hashed before and after); no unresolved `[[…]]`; Playwright (file://) renders of the summary tab at 390 and 1280 px show the new paragraphs with no horizontal overflow. Manuscript export not rerun: the exporter renders `index.md` only, which is unchanged. Unverified: original Visma PDFs (cached extractions only).

## In-depth review round 5 (23 September 2026): dispositions

Implemented from the run `.in-depth-reviews/20260923-160608.PjF9JD` (round-03 review). The review reported no actionable findings; its terminology audit found every term explained where it is used. No article, companion, spec or image file was changed, and no build was needed: the built pages already matched the sources.

| Finding | Disposition |
| --- | --- |
| None reported | **Nothing to fix.** Verified the state the review described instead: all 16 inventory files match the review’s SHA-256 hashes; the round-4 summary revision (VISMA-006) is in place at 500 prose words. |

Checks run: embedded article, TL;DR, comic and spec bodies in `docs/private-techuity/visma.html` and `visma.spec.html` contain every source sentence (cross-links rewritten, nothing missing); no unresolved `[[…]]`; every local `src`/`href` resolves; all ten images under `docs/private-techuity/assets/images/27-visma/` are byte-identical to the sources; the six comic-panel metadata blocks parse, their captions and alt texts equal the rendered lines and their recorded hashes match the files; 892.646 + 11.665 = 904.311, 892.6 + 11.7 = 904.3, 33 ÷ 12 = 2.75, 737 ÷ 649 − 1 ≈ 13.6%. Fresh Playwright (file://) renders of the Article, TL;DR and Comic tabs at 390 px and 1280 px: no horizontal overflow, no broken images, no console errors (this closes the review’s rendering gap). Manuscript untouched (sources unchanged). Still unverified: the three original Visma PDFs (cached text extractions only); the Harvard case contents.
