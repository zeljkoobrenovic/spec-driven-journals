...
Requests of that kind reach a company leader in three forms that sound alike and are not. A **formal approval right** comes from the agreements: a reserved matter, a board seat, a consent over executive pay or the operating plan. The response is to bring the authorized body a decision it can take. A **funding condition** attaches money to a plan: a bridge drawn only once a cost plan is adopted, a round whose term sheet wants a particular executive in place. The response is to translate the condition into a date and a number, and decide whether to meet it. **Influence** is everything else: an adviser’s benchmark, a director’s view of a leader, an introduction. The response is to record it as an option and assess it on the same evidence as any other. [[fix-decisions-before-hiring]] works an investor-proposed appointment through that distinction, and [[headcount-plan-shrinks]] a reduction, where all three arrive at the same board meeting.

## Make a Conditional Commitment Explicit, and Plan the Missed Deadline

Back to the onboarding proposal. The board answered within the two weeks. It did not approve the €1 million program in one year. It endorsed the direction and confirmed the first stage Ines had already approved within her delegation in [[obligations-before-budget]]: the reusable setup step as a pilot, €180,000 for the build plus €30,000 of first-year maintenance, €210,000 from the year’s residual and opening cash, and twelve engineer-weeks of the current team. It kept the two implementation hires as its own decision, because permanent hires outside the approved headcount plan are the board’s whatever they cost.
