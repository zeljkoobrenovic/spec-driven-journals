...
Usage from the Spec-Driven Journals project root:

    GEMINI_API_KEY=... python3 _journals/spec-driven-journals/posts/generate_illustrations_nanobanana.py --dry-run
    GEMINI_API_KEY=... python3 _journals/spec-driven-journals/posts/generate_illustrations_nanobanana.py --overwrite --only journal-build-pipeline.png
    GEMINI_API_KEY=... python3 _journals/spec-driven-journals/posts/generate_illustrations_nanobanana.py --post anatomy-of-a-journal
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Only generate filenames or image sources matching this value. "
            "Can be provided multiple times. Example: --only journal-build-pipeline.png"
        ),
    )
    parser.add_argument(
        "--post",
        ...
        default=[],
        help=(
            "Only generate illustrations for a post folder or permalink. "
...
def front_matter_value(fm_lines: list[str], key: str, fallback: str) -> str:
...
def collect_context(lines: list[str], image_line_index: int, max_chars: int = 900) -> str:
    snippets: list[str] = []
    in_code_fence = False
    for i in range(max(0, image_line_index - 14), image_line_index):
        stripped = lines[i].strip()
        if stripped.startswith("```"):
        ...
        if in_code_fence or not stripped:
            continue
        if stripped.startswith(("---", "![", "|", "*Illustration placeholder:")):
            continue
        if stripped.startswith("#"):
...
def build_prompt(target: IllustrationTarget) -> str:
    return f"""
Create a polished article illustration for a technical journal named Spec-Driven Journals.
...
- use a polished editorial documentation style for software architecture and knowledge systems
- use flat vector-like shapes, crisp cards, arrows, folder trees, document icons, browser pages, and workflow lines where useful
- use a restrained professional palette: soft blue, teal, slate, and small orange accents
- make any labels short, large, and readable; avoid dense text and tiny labels
- include no real company logos, no trademarks, no watermark, and no fake noisy screenshots
- make the image useful as an explanatory figure in an article, not as decoration
- keep all important content inside generous margins
- visually communicate source-first authoring, specs, generated documents, review loops, and AI-assisted maintenance when relevant
""".strip()
...
def discover_targets() -> list[IllustrationTarget]:
    ...
    for post_path in sorted(SCRIPT_DIR.glob("*/index.md")):
        ...
        post_title = front_matter_value(fm_lines, "title", post_slug)
        permalink = front_matter_value(fm_lines, "permalink", post_slug)
        ...
        for i, line in enumerate(lines):
            ...
            if in_code_fence:
                continue
            match = IMAGE_REF_RE.match(stripped)
            if not match:
                continue
            src = match.group("src")
...
def filter_targets(targets: list[IllustrationTarget], args: argparse.Namespace) -> list[IllustrationTarget]:
    ...
    if posts:
        targets = [target for target in targets if target.post_slug in posts or target.permalink in posts]
    return targets
...
def main(argv: list[str] | None = None) -> int:
    ...
    targets = filter_targets(discover_targets(), args)
    ...
    for target in targets:
        ...
        if args.limit and generated >= args.limit:
            print("limit reached; remaining targets not generated.")
            break
