...
``sections[].posts`` is checked. For each post that does not already have a
``logo:`` front-matter field, the script:

  1. Builds a short visual prompt from the post title + excerpt + first body
     paragraphs.
...
def build_prompt(title: str, excerpt: str, body: str) -> str:
    ...
    parts = [
        ...
        "Wide landscape framing (16:9 aspect ratio) suitable for use as a "
        "post cover image.",
        "",
...
def process_post(journal_dir: Path, post_src: Path, api_key: str, dry_run: bool) -> str:
    ...
    if meta.get("logo"):
        return f"skip (logo already set): {label}"
    ...
    if image_path.exists():
        ...
        if not dry_run:
            write_post_with_added_fm(
                ...
                fm_lines,
                body,
                [("logo", logo_value), ("logo_credit", LOGO_CREDIT)],
            )
